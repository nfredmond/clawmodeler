...
- **DOCX report rendering.** `clawmodeler-engine export --format docx` and `workflow full/report-only --format docx` now produce Word-compatible `.docx` Planner Pack reports for planners who live in Word. Headings, paragraphs, bold/italic/code emphasis, bullet and numbered lists, tables, and blockquotes are preserved; relative figure paths embed as images when the files exist. The installer ships with the `[pdf,docx]` extras bundled; devs installing from source use `pip install "clawmodeler-engine[pdf,docx]"`.
...
- **Bundled PDF runtime.** Release sidecars now install `[pdf,docx]`, bundle the macOS/Windows WeasyPrint native runtime beside `clawmodeler-engine`, and configure the frozen launcher so PDF export works from the installer instead of only from developer shells with system libraries.
- **Sidecar smoke gate now exercises PDF and DOCX export** against the built binary, including PDF `%PDF-` magic-header validation, to prevent installers from shipping broken optional-deps. Previously PDF was broken in v0.9.6 installers because `release:sidecar-smoke` only tested the default Markdown path.
