...
The current planner-facing flow is guided by a top-level Workflow Guide. It links to the existing panels and refreshes `clawmodeler-engine data index` when a workspace is loaded, then derives run, artifact, QA, bridge readiness, Planner Pack, portfolio, and diff summaries from `logs/workspace_index.json` with direct artifact reads as fallback. Recent workspaces, active run history, and planner-facing run labels are persisted locally in the desktop workbench:
...
The first-user release-candidate gate is:
