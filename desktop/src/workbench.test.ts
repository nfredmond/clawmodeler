...
describe("clawmodeler workbench helpers", () => {
  ...
  it("normalizes run summary details from workspace artifacts", () => {
    const summary = summarizeRunArtifacts({
      ...
      workflowReport: {
        ...
        routing: {
          ...
          proxy_comparison: {
            network_engine: "network_edges_dijkstra",
            coverage_status: "complete",
            coverage_detail: "Selected network routing reaches every ordered zone pair.",
            zone_count: 3,
            compared_pairs: 6,
            reachable_pairs: 6,
            unreachable_pairs: 0,
            reachable_pair_share: 1,
            mean_network_minutes: 16.333,
            mean_proxy_minutes: 3.705,
            mean_abs_delta_minutes: 12.628,
            max_abs_delta_minutes: 20.295,
            proxy_speed_kph: 40,
            note: "Screening diagnostic.",
          },
        },
      },
      reportMarkdown: null,
    ...
    expect(summary?.routing).toEqual({
      ...
      detail: "Auto-selection uses staged network_edges.csv.",
      proxyComparison: {
        networkEngine: "network_edges_dijkstra",
        coverageStatus: "complete",
        coverageDetail: "Selected network routing reaches every ordered zone pair.",
        zoneCount: 3,
        comparedPairs: 6,
        reachablePairs: 6,
        unreachablePairs: 0,
        reachablePairShare: 1,
        meanNetworkMinutes: 16.333,
        meanProxyMinutes: 3.705,
        meanAbsDeltaMinutes: 12.628,
        maxAbsDeltaMinutes: 20.295,
        proxySpeedKph: 40,
        note: "Screening diagnostic.",
      },
    });
